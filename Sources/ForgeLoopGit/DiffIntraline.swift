...
// Original: Shared/Sources/TermioShared/DiffIntraline.swift
// Copyright (c) 2026 Jiwei Yuan
// https://github.com/termio-sh/termio/blob/main/LICENSE
import Foundation

/// Marks the changed spans inside a modified line pair (Critique's and Xcode's
/// intraline highlight).
///
/// The spans come from a word-level diff, not from stripping the pair's common
/// prefix and suffix: a line with two separate edits — `foo(a, b)` → `bar(a, c)` —
/// has no common middle to strip, so prefix/suffix stripping has to emphasize
/// everything between the first and last change and the reader learns nothing.
/// Diffing the words finds both edits and leaves `(a, ` alone.
///
/// Offsets are `Character` counts, matching `DiffLine.text`.
public enum DiffIntraline {
    /// Longer lines are left plain rather than diffed — the pathological cases here
    /// are minified bundles and embedded data, where every span would be noise anyway.
    private static let maximumLineLength = 2000

    /// The changed spans of a deletion/addition line pair, or `nil` when the two
    /// share so little that spans would be noise (a rewritten line reads better as
    /// a plain add/delete pair than as one line-long highlight).
    public static func spans(old oldText: String, new newText: String)
        ...
        // "Too different to be worth marking": at least a fifth of the shorter line
        // must survive unchanged. Survival is measured per side against that side's
        // own length — charging the longer side's inserted characters against the
        // shorter line reads a pure insertion (where the old line survives whole) as
        // a rewrite and drops the highlight entirely.
    ...
    /// Splits a line into word runs, whitespace runs, and single characters. CJK
    /// scripts have no intra-word boundaries, so each ideograph or kana is its own
    /// token rather than being swallowed into a run that would span the whole line.
    ...
    /// stdlib's Myers diff answers exactly this, and answers it in O(ND).
    ...
    /// Contiguous runs of changed tokens, as character ranges. Runs separated only
    /// by unchanged whitespace are joined: `a = 1` → `b = 2` reads better as one
    /// span than as two boxes with a gap.
    private static func spans(of tokens: [Token], changed: [Bool]) -> [Range<Int>] {
        ...
        while index < tokens.count {
            ...
            // A span that opens or closes on whitespace would paint empty background
            // at its edge; trim it back to real content unless whitespace *is* the
            // change (an indent edit, where there is nothing else to mark).
